...
function gent_base_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  // the theme trail.
  foreach (gent_base_theme_trail() as $theme => $name) {
  ...
  $form['#attached']['css'][] = drupal_get_path('theme', 'gent_base') . '/css/gent_base.admin.css';
  $form['#attached']['js'][] = drupal_get_path('theme', 'gent_base') . '/js/gent_base.admin.min.js';
  ...
  if ($extensions = gent_base_extensions(NULL, TRUE)) {
    $form['gent_base'] = array(
      '#type' => 'vertical_tabs',
      '#weight' => -10,
    );
    ...
    foreach ($extensions as $extension => $info) {
      $form['gent_base'][$extension] = array(
        '#type' => 'fieldset',
        '#title' => $info['info']['name'],
        '#attributes' => array(
          'class' => array('gent_base-extension'),
        ),
      );
      ...
      if (!empty($info['info']['dependencies'])) {
        foreach ($info['info']['dependencies'] as $dependency) {
          ...
          elseif (($version = gent_base_check_incompatibility($dependency, $module['version'])) !== NULL) {
        ...
        if (!empty($errors)) {
          $form['gent_base'][$extension]['errors'] = array(
            '#type' => 'item',
            ...
            '#name' => 'gent_base-requirements',
          );
        }
      }

      // Disable all options if there were any errors.
      $form['gent_base'][$extension]['#disabled'] = !empty($errors) || variable_get('gent_base_toggle_extension_' . $extension) !== NULL;

      $form['gent_base'][$extension]['gent_base_toggle_extension_' . $extension] = array(
        '#type' => 'checkbox',
        '#title' => t('Enable @extension extension', array('@extension' => $info['info']['name'])) . (variable_get('gent_base_toggle_extension_' . $extension) !== NULL ? ' <span class="marker">(' . t('overridden') . ')</span>' : ''),
        '#description' => t('This setting can be overridden with an equally named variable (@name) so you can control it on a per-environment basis by setting it in your settings.php file.', array('@name' => 'gent_base_toggle_extension_' . $extension)),
        '#default_value' => gent_base_extension_enabled($extension),
        '#weight' => -10,
      );
      ...
      if (function_exists($function)) {
        ...
        $element = $function($element, $form, $form_state) + array(
          ...
          '#attributes' => array(
            'class' => array('gent_base-extension-settings')
          ),
          '#states' => array(
            'disabled' => array(
              'input[name="gent_base_toggle_extension_' . $extension . '"]' => array('checked' => FALSE),
            ),
          ),
        );
      }
 ...
 */
function gent_base_theme_settings_form_submit($form, &$form_state) {
  // Clear the theme cache.
  $theme = $form_state['build_info']['args'][0];
  cache_clear_all('gent_base:' . $theme . ':', 'cache', TRUE);

  // We also need to clear the static right away.
  drupal_static_reset('gent_base_extensions');
  ...
  foreach ($form_state['extensions'] as $extension) {
    if (!$form_state['values']['gent_base_toggle_extension_' . $extension]) {
      _gent_base_retain_extension_settings($form, $form_state, $extension, $theme);
    }
  }

  // This is a relict from the vertical tabs and should be removed so it doesn't
  // end up in the theme settings array.
  unset($form_state['values']['gent_base__active_tab']);
}

/**
 * Helper function for retaining settings of an extension.
 */
function _gent_base_retain_extension_settings($form, &$form_state, $extension, $theme, $parents = array()) {
  $current = array_merge(array('gent_base', $extension, 'settings'), $parents);

  if ($items = drupal_array_get_nested_value($form, $current)) {
    foreach (element_children($items) as $key) {
      if (array_key_exists($key, $form_state['values'])) {
        $form_state['values'][$key] = gent_base_theme_get_setting($key, NULL, $theme);
      }

      $next = array_merge($parents, array($key));
      _gent_base_retain_extension_settings($form, $form_state, $extension, $theme, $next);
    }
  }
}
